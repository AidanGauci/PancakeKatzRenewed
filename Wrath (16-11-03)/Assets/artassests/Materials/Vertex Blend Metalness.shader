Shader "Custom/Vertex Blending (Metalness Setup)"
{
	Properties
	{
		...
		_SpecGlossMap("Metalness(R), AO(g), Smoothness (G) ", 2D) = "white" {}
		...
		_SpecGlossMap2("Metalness(R), AO(g), Smoothness (G)", 2D) = "white" {}
		...
		_SpecGlossMap3("Metalness(R), AO(g), Smoothness (G)", Color) = (0,1,0.5,0) 
		...
		_SpecGlossMap4("Metalness(R), AO(g), Smoothness (G)", Color) = (0,1,0.5,0)
		_NormalMap4 ("NormalMap 4", 2D) = "bump" {}
		
		[Space(50)]_AOIntensity ("AO Intensity", Range(0,1)) = 0.0
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutputStandard o)
		{
			...
			spec1.rgb = spec1.rgb*blend2.x + spec2.rgb*blend2.y;
			spec1.rgb = spec1.rgb*blend3.x + _SpecGlossMap3.rgb*blend3.y;
			spec1.rgb = spec1.rgb*blend4.x + _SpecGlossMap4.rgb*blend4.y;
			...
			o.Metallic = spec1.r;
			o.Smoothness = spec1.b;
			o.Occlusion = lerp (1, spec1.g, _AOIntensity);
